...
def resize_bicubic_nhwc(x, scale_h, scale_w):
    """Channel-last ``F.interpolate(scale_factor=..., mode="bicubic")``: the
    output size is floored and sampling uses the given scale, not the effective one."""
    _, in_h, in_w, _ = x.shape
    iy, wy = _bicubic_weights_1d(in_h, int(in_h * scale_h), scale_h)
    ix, wx = _bicubic_weights_1d(in_w, int(in_w * scale_w), scale_w)
    return separable_interpolate(x, iy, wy, ix, wx)
